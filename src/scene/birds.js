// ── Birds — choughs on the summit thermal, gulls round the hull ─
...
export function createBirds(opts = {}) {
  const {
    center = [96, 366, 310],
    radii = [60, 95],
    window: win = [0.10, 0.20],   // fade-out window in progress space
    invert = false,               // true: fade IN over the window instead
    tint = [0.055, 0.06, 0.07],
    count = 16,
  } = opts;
  const N = count;
  ...
  const idx = [];
  for (let i = 0; i < N; i++) {
    const r = radii[0] + Math.random() * (radii[1] - radii[0]);
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */`
      ...
      const vec3 CENTER = vec3(${center[0].toFixed(1)}, ${center[1].toFixed(1)}, ${center[2].toFixed(1)});
      void main() {
        ...
        float w = smoothstep(${win[0].toFixed(3)}, ${win[1].toFixed(3)}, uProgress);
        vAlpha = ${invert ? 'w' : '(1.0 - w)'} * 0.85;
      }
    `,
    fragmentShader: /* glsl */`
      ...
      void main() {
        ...
        gl_FragColor = vec4(vec3(${tint[0].toFixed(3)}, ${tint[1].toFixed(3)}, ${tint[2].toFixed(3)}), vAlpha);
      }
    `,
  });
